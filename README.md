owjmowq4nwiymge@myr907.ml
Yttyhxways778!

rm -rf sess/tgup*
python3 sessgen.py
mv tgup* sess
heroku login -i
heroku container:login
heroku create owjmowq4nwiymge
heroku config:set luzhishichang=1800 ids_str= -a owjmowq4nwiymge

heroku config:get luzhishichang -a owjmowq4nwiymge >heroku_config.ini
heroku config:get ids_str -a owjmowq4nwiymge >>heroku_config.ini
git add -A && git commit -m "add dyzb26" && git push origin dyzb26
heroku container:push web -a owjmowq4nwiymge && heroku container:release web -a owjmowq4nwiymge