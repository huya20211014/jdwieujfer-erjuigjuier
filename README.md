ogmzymi1njgyndy@dslab2022.tk
Yttyhxways778!

mkdir record
mkdir recordok
touch record/1.txt
touch recordok/1.txt
rm -rf sess/tgup*
python3 sessgen.py
mv tgup* sess
heroku login -i
heroku container:login
heroku create ogmzymi1njgyndy
heroku config:set luzhishichang=1800 ids_str= -a ogmzymi1njgyndy

heroku config:get luzhishichang -a ogmzymi1njgyndy >heroku_config.ini
heroku config:get ids_str -a ogmzymi1njgyndy >>heroku_config.ini
git add -A && git commit -m "add dyzbxby26" && git push origin dyzbxby26
heroku container:push web -a ogmzymi1njgyndy && heroku container:release web -a ogmzymi1njgyndy